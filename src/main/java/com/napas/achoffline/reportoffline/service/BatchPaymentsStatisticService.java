...
@Service
@Slf4j
public class BatchPaymentsStatisticService {

    @Autowired
    private HisBatchPaymentsDAO hisBatchPaymentsDAO;


    public void getBatchPaymentExcelStream(HttpServletResponse response, String beginDate, String endDate,
        ...
        try {
            ...
            List<BatchPaymentsStatistic> list = new ArrayList<>();
            if(reportType.equals("0")) {
                ...
                for (LocalDate begin = localDateBegin; begin.compareTo(localDateEnd) <= 0; begin = begin.plusDays(1)) {
                    List<BatchPaymentsStatistic> temp = hisBatchPaymentsDAO.findStatisticBatchPayments(begin.getYear(),
                            begin.getMonthValue(), begin.getDayOfMonth(), listBoc, listTtc, listTransType, listCreditorType,
                            listTransStatus, listInstructing, beginDateSearch, endDateSearch);
            ...
            list = list.stream().sorted(Comparator.comparing(BatchPaymentsStatistic::getConvertDate)).collect(Collectors.toList());
            BatchPaymentsStatisticExporter exporter = new BatchPaymentsStatisticExporter(list,reportType);
