...
import com.napas.achoffline.reportoffline.service.TblTariffPlanService;
 ...
 */
@RestController
@RequestMapping("/api/TblTariffPlan")
public class TblTariffPlanController {

    @Autowired
    private TblTariffPlanService tblTariffPlanService;
    ...
    public List<TblTariffPlanDTO> list(HttpServletRequest request) {
        ...
        return tblTariffPlanService.list();
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('ROOT', 'ADMIN', 'SENIOR_OPERATOR', 'TRA_SOAT', 'KIEM_SOAT')")
    public TblTariffPlanDTO get(HttpServletRequest request,@PathVariable int id) {
        Principal principal = (Principal) SecurityContextHolder.getContext().getAuthentication();
        hisAccessService.logAccess(request.getMethod(), request.getRemoteAddr(),
                principal.getName(), request.getRequestURI(), "","");
        return tblTariffPlanService.get(id);
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('ROOT', 'ADMIN', 'SENIOR_OPERATOR')")
    public ResponseEntity<?> put(HttpServletRequest request1,@PathVariable int id, @Valid @RequestBody TblTariffPlanDTO input) throws UnsupportedEncodingException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        Principal principal = (Principal) SecurityContextHolder.getContext().getAuthentication();
        ...
        if (checkSignature(request1)){
            return tblTariffPlanService.put(id, input);
        } else return ResponseEntity.ok("Sai chữ ký");
    }

    @PostMapping
    @PreAuthorize("hasAnyAuthority('ROOT', 'ADMIN', 'SENIOR_OPERATOR')")
    public ResponseEntity<?> post(HttpServletRequest request1, @Valid @RequestBody TblTariffPlanDTO input) throws UnsupportedEncodingException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        Principal principal = (Principal) SecurityContextHolder.getContext().getAuthentication();
        hisAccessService.logAccess(request1.getMethod(), request1.getRemoteAddr(),
                principal.getName(), request1.getRequestURI(), "",input.toString());
        if (checkSignature(request1)){
            return tblTariffPlanService.post(input);
        } else return ResponseEntity.ok("Sai chữ ký");
    ...
    public ResponseEntity<?> delete(HttpServletRequest request,@PathVariable int id) {
        ...
        hisAccessService.logAccess(request.getMethod(), request.getRemoteAddr(),
                principal.getName(), request.getRequestURI(), "","");
        return tblTariffPlanService.delete(id);
    }
