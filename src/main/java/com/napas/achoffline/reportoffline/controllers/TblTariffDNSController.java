...
import com.napas.achoffline.reportoffline.service.TblTariffDNSService;
 ...
 */
@RestController
@RequestMapping("/api/TblTariffDns")//dns
public class TblTariffDNSController {

    @Autowired
    private TblTariffDNSService tblTariffDNSService;
    ...
    public List<TblTariffDNSDTO> find(HttpServletRequest request, @RequestParam(name = "tariffLadderId") int tariffLadderId) {
        ...
        return tblTariffDNSService.find(tariffLadderId);
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('ROOT', 'ADMIN', 'SENIOR_OPERATOR', 'TRA_SOAT', 'KIEM_SOAT')")
    public TblTariffDNSDTO get(HttpServletRequest request,@PathVariable int id) {
        Principal principal = (Principal) SecurityContextHolder.getContext().getAuthentication();
        hisAccessService.logAccess(request.getMethod(), request.getRemoteAddr(),
                principal.getName(), request.getRequestURI(), "","");
    ...
    public ResponseEntity<?> put(HttpServletRequest request1,@PathVariable int id, @RequestBody TblTariffDNSDTO input) throws UnsupportedEncodingException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        Principal principal = (Principal) SecurityContextHolder.getContext().getAuthentication();
    ...
    @PostMapping
    @PreAuthorize("hasAnyAuthority('ROOT', 'ADMIN', 'SENIOR_OPERATOR')")
    public ResponseEntity<?> post(HttpServletRequest request1,@RequestBody TblTariffDNSDTO input) throws UnsupportedEncodingException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        Principal principal = (Principal) SecurityContextHolder.getContext().getAuthentication();
        hisAccessService.logAccess(request1.getMethod(), request1.getRemoteAddr(),
                principal.getName(), request1.getRequestURI(), "",input.toString());
        if (checkSignature(request1)){
