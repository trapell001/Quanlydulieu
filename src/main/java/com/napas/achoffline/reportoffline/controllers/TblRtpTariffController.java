...
import com.napas.achoffline.reportoffline.service.TblRtpTariffService;
 ...
 */
@RestController
@RequestMapping("/api/TblTariffRtp")
public class TblRtpTariffController {

    @Autowired
    private TblRtpTariffService tblTariffService;
    ...
    public List<TblRtpTariffDTO> find(HttpServletRequest request,@RequestParam(name = "tariffLadderId") int tariffLadderId) {
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('ROOT', 'ADMIN', 'SENIOR_OPERATOR', 'TRA_SOAT', 'KIEM_SOAT')")
    public TblRtpTariffDTO get(HttpServletRequest request,@PathVariable int id) {
        Principal principal = (Principal) SecurityContextHolder.getContext().getAuthentication();
        hisAccessService.logAccess(request.getMethod(), request.getRemoteAddr(),
                principal.getName(), request.getRequestURI(), "","");
    ...
    public ResponseEntity<?> put(HttpServletRequest request1,@PathVariable int id, @RequestBody TblRtpTariffDTO input) throws UnsupportedEncodingException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        Principal principal = (Principal) SecurityContextHolder.getContext().getAuthentication();
    ...
    @PostMapping
    @PreAuthorize("hasAnyAuthority('ROOT', 'ADMIN', 'SENIOR_OPERATOR')")
    public ResponseEntity<?> post(HttpServletRequest request1,@RequestBody TblRtpTariffDTO input) throws UnsupportedEncodingException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        Principal principal = (Principal) SecurityContextHolder.getContext().getAuthentication();
        hisAccessService.logAccess(request1.getMethod(), request1.getRemoteAddr(),
                principal.getName(), request1.getRequestURI(), "",input.toString());
        if (checkSignature(request1)){
