 ...
 */
@Component
public class ReportOfflineDAO {

    @Autowired
    protected DataSource localDataSource;

    public JasperPrint getReport(
            ReportOfflineDef reportDef,
        ...
        Map<ReportOfflineFunctionParameter, Boolean> mapParamsDef = reportDef.getParameters();
        ...
        try (Connection connection = localDataSource.getConnection();
            ...
            if (mapParamsDef.get(ReportOfflineFunctionParameter.DATE_BEGIN)) {
                stmt.setDate(paramIndex++, filter.getDateBegin() != null ? Date.valueOf(filter.getDateBegin()) : null);
            }

            if (mapParamsDef.get(ReportOfflineFunctionParameter.DATE_END)) {
                stmt.setDate(paramIndex++, filter.getDateEnd() != null ? Date.valueOf(filter.getDateEnd()) : null);
            }

            if (mapParamsDef.get(ReportOfflineFunctionParameter.SESSION_BEGIN)) {
                stmt.setString(paramIndex++, filter.getSessionBegin());
            }

            if (mapParamsDef.get(ReportOfflineFunctionParameter.SESSION_END)) {
                stmt.setString(paramIndex++, filter.getSessionEnd());
            }

            if (mapParamsDef.get(ReportOfflineFunctionParameter.BIC)) {
                stmt.setString(paramIndex++, paramBic);
            }

            if (mapParamsDef.get(ReportOfflineFunctionParameter.BOC)) {
                stmt.setString(paramIndex++, filter.getBoc() != null ? filter.getBoc().toString() : "");
            }

            if (mapParamsDef.get(ReportOfflineFunctionParameter.TTC)) {
                stmt.setString(paramIndex++, filter.getTtc());
            }

            if (mapParamsDef.get(ReportOfflineFunctionParameter.PARTICIPANT_TYPE)) {
                stmt.setString(paramIndex++, filter.getParticipantType() != null ? filter.getParticipantType().toString() : "");
            }

            if (mapParamsDef.get(ReportOfflineFunctionParameter.TRANS_TYPE)) {
                stmt.setString(paramIndex++, filter.getTransType());
            }

            if (mapParamsDef.get(ReportOfflineFunctionParameter.PARTICIPAN_ROLE)) {
